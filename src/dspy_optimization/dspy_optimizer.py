...
def check_dspy_optimization(model_name, dataset_dict, output_path="models/dspy_compiled_program.json"):
    """
    Runs DSPy optimization to create an optimized prompt program.
    """
    print(f"Setting up DSPy with model: {model_name}...")
    
    # Configure DSPy with the local Hugging Face model
    # Note: This loads the model. 
    lm = dspy.HFModel(model=model_name)
